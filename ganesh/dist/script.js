...
function onWindowResize() {
    ...
    // Mobile Responsive Logic: Camera FOV adjust kela
    if (WIDTH < HEIGHT) {
        camera.fov = 75; 
    } else {
        camera.fov = 60;
...
function handleTouchMove(event) {
    ...
    event.preventDefault(); // Scroll thambvnyasathi
}
...
function createLion(){
    lion = new Lion();
    // Mobile sathi lion cha scale
    if (window.innerWidth < 480) {
        lion.threegroup.scale.set(0.7, 0.7, 0.7);
        lion.threegroup.position.y = -30;
    }
...
function createFan(){
    ...
    // Responsive Fan scale
    if (window.innerWidth <= 480) {
        fan.threegroup.scale.set(0.4, 0.4, 0.4);
        fan.threegroup.position.z = 150;
    } else if (window.innerWidth <= 768) {
        fan.threegroup.scale.set(0.6, 0.6, 0.6);
        fan.threegroup.position.z = 210;
    }
